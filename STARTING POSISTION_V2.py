...
k = 8                               # number of paths
gamma = 0.2                        # constant that determines non linearity of radius response to flow. Present in dRdt function.         
...
def Adjust_radius(flow_per_tube):
    ...
    for u,v in G.edges():
        G[u][v]['radius'] = (G[u][v]['radius']*(1+(flow_per_tube.get((u,v))**gamma/(1+flow_per_tube.get((u,v))**gamma)))) - dR     #scaled to r
        if G[u][v]['radius'] < 0:
